...
function Signup() {
  ...
  const [success, setSuccess] = useState(false);
  const [status, setStatus] = useState(false);
  // State to track if the file is still loading.
  ...
  const handleSubmit = async (e) => {
    ...
    // Check if the image is still loading or is empty.
    if (isImageLoading) {
      alert("Please wait until the image has finished processing.");
      return;
    }
    if (!formData.image) {
  ...
  return (
    <div className="flex flex-col relative top-8 xs:top-20 xl:top-10 items-center justify-center bg-white drop-shadow-2xl shadow shadow-blue-300 w-[370px] xs:w-[400px] sm:w-[450px] rounded-md">
      ...
      <form onSubmit={handleSubmit} className='flex flex-col p-5 mt-3 font-Montserrat'>
        ...
        <input 
          ...
          onChange={(e) => {
            ...
            if (file) {
              ...
              setIsImageLoading(true); // Disable submit while reading file
              const reader = new FileReader();
              reader.onloadend = () => {
                setFormData({ ...formData, image: reader.result });
                setIsImageLoading(false); // Re-enable submit once done
              };
              reader.readAsDataURL(file);
            }
          }}
        ...
        <button 
          type="submit" 
          disabled={isImageLoading}  // Disable submit button while image loads
